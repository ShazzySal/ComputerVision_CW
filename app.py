...
def assess_image_quality(img: np.ndarray) -> dict:
    ...
    # 4. Retina disk presence heuristic (green channel energy in center)
    h, w = gray.shape
    center_patch = img[h//4:3*h//4, w//4:3*w//4] if img.ndim == 3 else gray[h//4:3*h//4, w//4:3*w//4]
    ...
    passed = len(issues) == 0
    return {"passed": passed, "issues": issues, "warnings": warnings, "blur_score": lap_var, "brightness": mean_brightness, "black_ratio": black_ratio}
...
def build_quality_warning_html(qc: dict) -> str:
    ...
    if qc["passed"] and not qc["warnings"]:
        return (
            ...
            f'<span style="color:#64748b; font-size:12px; margin-left:10px;">Blur score: {qc["blur_score"]:.1f} | Brightness: {qc["brightness"]:.0f}/255 | Black border: {qc["black_ratio"]*100:.1f}%</span>'
            '</div>'
        )
    ...
    return (
        ...
        f'<div style="font-size:11px; color:#64748b; margin-top:4px;">Blur score: {qc["blur_score"]:.1f} | Brightness: {qc["brightness"]:.0f}/255 | Black border: {qc["black_ratio"]*100:.1f}%</div>'
        '</div>'
    )
